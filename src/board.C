...
BOARD_DATA boards[MAX_BOARD] =
{
  ...
  { "General",  	"General discussion",			0,	2,    	
    "all",	DEF_INCLUDE,	21, NULL, FALSE },
  { "Welcome",	        "Notes to newcomers",			0,	IM+5,	
    "all",	DEF_INCLUDE, 	365,NULL, FALSE	},
  { "Ideas",	        "Suggestion for improvement",		0,	2,	
    "all",	DEF_NORMAL,	60, NULL, FALSE }, 
  { "Announce", 	"Announcements from Immortals",		0,	IM,	
    "all",	DEF_NORMAL,	60, NULL, FALSE },
  { "Bugs",	        "Typos, bugs, errors",			0,	1,     	
    "imm",	DEF_NORMAL,	60, NULL, FALSE },
  { "Personal", 	"Personal messages",			0,	1,     	
    "all",	DEF_EXCLUDE,	28, NULL, FALSE },
  { "Penalties",	"Penalties to players",			0,	IM,	
    "all",	DEF_NORMAL,	60, NULL, FALSE },
  { "Immortals",	"Topics : OLC, admin, building...",	IM,	IM,	
    "imm",	DEF_NORMAL,	90, NULL, FALSE }
};
...
{
  char buf[MAX_STRING_LENGTH*9]; // 9*4608 > 32768

  /* Ugly colors ? */	
  sprintf (buf,
	   "[" BOLD "%4d" NO_COLOR "] " BOLD_YELLOW "%s" NO_COLOR ": " GREEN "%s" NO_COLOR "\n\r"
	   BOLD_YELLOW "Date" NO_COLOR ":  %s\n\r"
	   BOLD_YELLOW "To" NO_COLOR ":    %s\n\r"
	   GREEN "===========================================================================" NO_COLOR "\n\r"
	   "%s\n\r",
...
int unread_notes (CHAR_DATA *ch, BOARD_DATA *board)
{
  ...
  for (note = board->note_first; note; note = note->next)
    if (str_cmp(note->sender,ch->name) && is_note_to(ch, note) && ((long)last_read < (long)note->date_stamp) )
      count++;
...
void do_nwrite (CHAR_DATA *ch, const char *argument)
{
  ...
  act (BOLD_GREEN "$n starts writing a note." NO_COLOR , ch, NULL, NULL, TO_ROOM);
  ...
  sprintf (buf, BOLD_YELLOW "From" NO_COLOR ":    %s\n\r\n\r", ch->name);
  send_to_char (buf,ch);
  ...
  if (!ch->pcdata->in_progress->text) { /* Are we continuing an old note or not? */
    ...
    case DEF_NORMAL:
      ...
      break;
    case DEF_INCLUDE:
      if (!str_cmp(ch->pcdata->board->names,"all")) {
	/* Any way, everyone will get this... skip to field */
	send_to_char ("\n\r" BOLD_YELLOW "To" NO_COLOR ":      all (forced in this board)",ch);
	ch->pcdata->in_progress->to_list = str_dup ("all");

	send_to_char (BOLD_YELLOW "\n\rSubject" NO_COLOR ": ",ch);
    ...
    send_to_char (buf,ch);
    send_to_char ("\n\r" BOLD_YELLOW "To" NO_COLOR ":      ",ch);
	
    ch->desc->connected = CON_NOTE_TO;
    /* Oxtal */ SET_BIT(ch->comm,COMM_AFK);
    /* nanny takes over from here */
		
  }
  else {/* we are continuing, print out all the fields and the note so far*/
    sprintf (buf, BOLD_YELLOW "To" NO_COLOR ":      %s\n\r"
	     BOLD_YELLOW "Expires" NO_COLOR ": %s\n\r"
	     BOLD_YELLOW "Subject" NO_COLOR ": %s\n\r", 
	     ch->pcdata->in_progress->to_list,
	     ctime(&ch->pcdata->in_progress->expire),
	     ch->pcdata->in_progress->subject);
    send_to_char (buf,ch);
    send_to_char (BOLD_GREEN "Your note so far:\n\r" NO_COLOR,ch);
    send_to_char (ch->pcdata->in_progress->text,ch);
		
    send_to_char ("\n\rEnter text. Type " BOLD "END" NO_COLOR " on an empty line to end note.\n\r"
		  "Type " BOLD "-" NO_COLOR " or " BOLD "del" NO_COLOR " on an empty line to delete last line entered.\n\r"
		  "=======================================================\n\r",ch);
...
static void do_nread (CHAR_DATA *ch, const char *argument)
{
  ...
  if (!str_cmp(argument, "again")) { /* read last note again */
    ...
    for (p = ch->pcdata->board->note_first; p ; p = p->next, count++)
      if (str_cmp(p->sender,ch->name) && (p->date_stamp > *last_note) && is_note_to(ch,p)) {
	show_note_to_char (ch,p,count);
...
static void do_nlist (CHAR_DATA *ch, const char *argument)
{
  ...
  for (p = ch->pcdata->board->note_first; p; p = p->next)	{
	...
	sprintf (buf, BOLD "%3d" NO_COLOR ">" BOLD_BLUE "%c" BOLD_YELLOW "%-13s" NO_COLOR YELLOW " %s" NO_COLOR " \n\r",
		 num, 
...
void do_board (CHAR_DATA *ch, const char *argument)
{
  ...
  if (!argument[0]) {/* show boards */
    ...
    count = 1;
    send_to_char (BOLD_RED "Num         Name Unread Description" NO_COLOR "\n\r"
		  RED "=== ============ ====== ===========" NO_COLOR "\n\r",ch);
    for (i = 0; i < MAX_BOARD; i++)	{
      unread = unread_notes (ch,&boards[i]); /* how many unread notes? */
      if (unread != BOARD_NOACCESS) { /* watch out for the non-portable &%c !!! */
	sprintf (buf, BOLD "%2d" NO_COLOR "> " BOLD_GREEN "%12s" NO_COLOR " [{%c%4d" NO_COLOR "] " YELLOW "%s" NO_COLOR "\n\r", 
...
/* tries to change to the next accessible board with unread notes -- */
static bool next_board (CHAR_DATA *ch)
{
  int i = board_number (ch->pcdata->board) + 1;
	
  while ((i < MAX_BOARD) && (unread_notes(ch,&boards[i]) <= 0))
    i++;
  ...
  case DEF_EXCLUDE: /* forced exclude */
    if (is_full_name (ch->pcdata->board->names, buf)) {
      sprintf (buf, "You are not allowed to send notes to %s on this board. Try again.\n\r"
	       BOLD_YELLOW "To" NO_COLOR ":      ", ch->pcdata->board->names);
      write_to_char (d, buf);
...
void handle_con_note_subject (DESCRIPTOR_DATA *d, const char * argument)
{
  ...
  if (!buf[0]) {
    write_to_char (d, "Please find a meaningful subject!\n\r");
    write_to_char (d, BOLD_YELLOW "Subject" NO_COLOR ": ");
  }
  else  if (strlen(buf)>60) {
    write_to_char (d, "No, no. This is just the Subject. You're note writing the note yet. Twit.\n\r");
  }
...
void handle_con_note_expire(DESCRIPTOR_DATA *d, const char * argument)
{
  ...
  if (!buf[0]) /* assume default expire */
    ...
    if (!is_number(buf)) {
      write_to_char (d,"Write the number of days!\n\r");
      write_to_char (d,BOLD_YELLOW "Expire" NO_COLOR ":  ");
      return;
    }
    else {
      days = atoi (buf);
      if (days <= 0) {
	write_to_char (d, "This is a positive MUD. Use positive numbers only! :)\n\r");
	write_to_char (d, BOLD_YELLOW "Expire" NO_COLOR ":  ");
	return;
      }
    }
...
void handle_con_note_text (DESCRIPTOR_DATA *d, const char * argument)
{
  CHAR_DATA *ch = d->character;
  char buf[MAX_STRING_LENGTH];
  char letter[MAX_STRING_LENGTH*9]; // 9*4608 > 32768
  ...
  if ((!str_cmp(buf, "~")) || (!str_cmp(buf, "end"))) {
    write_to_char (d, "\n\r\n\r");
    write_to_char (d, szFinishPrompt);
    write_to_char (d, "\n\r");
  ...
  if (strlen (buf) > MAX_LINE_LENGTH) {
    write_to_char (d, "Too long line rejected. Do NOT go over 80 characters!\n\r");
    return;
  }
  ...
  /* Code for deleting line, by Oxtal */
  /* Modified by SinaC 2001
  if (   (   !str_cmp(buf,"-") 
	     || !str_cmp(buf,"del") ) 
	 && strlen(letter) > 2 ) {   
    // Remove a line
    letter[strlen(letter)-1] = 0; // I love C!
    *(strrchr(letter,'\n')+1) = 0;
...
void handle_con_note_finish (DESCRIPTOR_DATA *d, const char * argument)
{
  ...
  case 'c': /* keep writing */
    write_to_char (d,"Continuing note...\n\r");
    d->connected = CON_NOTE_TEXT;
    break;

  case 'v': /* view note so far */
    if (ch->pcdata->in_progress->text) {
      write_to_char (d,GREEN "Text of your note so far:" NO_COLOR "\n\r");
      write_to_char (d, ch->pcdata->in_progress->text);
    }
    else
      write_to_char (d,"You haven't written a thing!\n\r\n\r");
    write_to_char (d, szFinishPrompt);
    write_to_char (d, "\n\r");
    break;
  ...
  case 'p': /* post note */
    ...
    write_to_char (d, "Note posted.\n\r");
    for ( DESCRIPTOR_DATA *desc = descriptor_list; desc; desc = desc->next ) // SinaC 2003
      if ( desc->connected == CON_PLAYING
	   && is_note_to( desc->character, ch->pcdata->in_progress ) )
	send_to_charf(desc->character,
		      "{GA new note have been posted in board %s.{x\n\r",
		     ch->pcdata->board->short_name );
    d->connected = CON_PLAYING;
    /* remove AFK status */
    /* Oxtal */ REMOVE_BIT(ch->comm,COMM_AFK);

    // Added by SinaC 2003
    if (buf_string(ch->pcdata->buffer)[0] != '\0' )
      send_to_char("{rYou have received tells: Type 'replay' to see them.{x\n\r",ch);

    ch->pcdata->in_progress = NULL;
    // Modified by SinaC 2001
    //act (BOLD_GREEN "$n finishes $s note." NO_COLOR , ch, NULL, NULL, TO_ROOM);
    break;
				
  case 'f':
    write_to_char (d, "Note cancelled!\n\r");
    ch->pcdata->in_progress = NULL;
    d->connected = CON_PLAYING;
    /* remove afk status */
    /* Oxtal */ REMOVE_BIT(ch->comm,COMM_AFK);
  ...
  default: /* invalid response */
    write_to_char (d, "Huh? Valid answers are:\n\r\n\r");
    write_to_char (d, szFinishPrompt);
    write_to_char (d, "\n\r");
