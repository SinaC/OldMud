  ...
  for ( ; ; ) {
    ...
    if ( room != NULL )
      if ( can_see_room(ch,room)
	   ...
	   && !IS_SET(room->cstat(flags), ROOM_PRIVATE)
	   && !IS_SET(room->cstat(flags), ROOM_SOLITARY) 
	   && !IS_SET(room->cstat(flags), ROOM_SAFE)
	   && ( IS_NPC(ch) 
		|| IS_SET(ch->act,ACT_AGGRESSIVE) 
		|| !IS_SET(room->cstat(flags),ROOM_LAW) 
		// Added by SinaC 2001
...
void do_enter( CHAR_DATA *ch, const char *argument) {    
  ...
  if (argument[0] != '\0') {
    ...
    if ( portal == NULL )
      // Modified by SinaC 2001, damn, I was really stupid in 1997 :))
    ...
    if ( !IS_TRUSTED(ch,ANGEL) 
	 ...
	 && ( IS_AFFECTED(ch,AFF_CURSE) 
	      // Modified by SinaC 2001
	      ||   IS_SET(old_room->cstat(flags),ROOM_NO_RECALL))) {
    ...
    if (IS_SET(portal->value[2],GATE_RANDOM) || portal->value[3] == -1) {
      ...
      portal->baseval[3] = location->vnum; /* for record keeping :) */
      recompobj(portal);
    }
    ...
    if ( IS_NPC(ch) 
	 && IS_SET(ch->act,ACT_AGGRESSIVE)
	 // Modified by SinaC 2001
	 && IS_SET(location->cstat(flags),ROOM_LAW)) {
      send_to_char("Something prevents you from leaving...\n\r",ch);
      return;
    }

    // Added by SinaC 2001  to avoid people with a too big size
    if ( !IS_IMMORTAL(ch) 
	 // Modified by SinaC 2001
	 && location->cstat(maxsize) != SIZE_NOSIZE
	 && ( location->cstat(maxsize) < ch->cstat(size) 
	      /*|| pexit->max_size < ch->cstat(size)*/) ) {
      send_to_char( "You're too huge to enter that.\n\r", ch );
      return;
    }
