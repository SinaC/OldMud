...
void do_heal(CHAR_DATA *ch, const char *argument)
{
  ...
  if (arg[0] == '\0') {
    ...
    send_to_char("  psp:  restore psp 	          10 gold\n\r",ch);
  ...
  if (!str_prefix(arg,"light")) {
    ...
    sn    = ability_lookup("cure light");
    words = "judicandus dies";
    cost  = 1000;
  }
  else if (!str_prefix(arg,"serious")) {
    spell = spell_cure_serious;
    sn    = ability_lookup("cure serious");
    words = "judicandus gzfuajg";
    cost  = 1600;
  }
  else if (!str_prefix(arg,"critical")) {
    spell = spell_cure_critical;
    sn    = ability_lookup("cure critical");
    words = "judicandus qfuhuqar";
    cost  = 2500;
  }
  else if (!str_prefix(arg,"heal")) {
    spell = spell_heal;
    sn = ability_lookup("heal");
    words = "pzar";
    cost  = 5000;
  }
  else if (!str_prefix(arg,"blindness")) {
    spell = spell_cure_blindness;
    sn    = ability_lookup("cure blindness");
    words = "judicandus noselacri";		
    cost  = 2000;
  }
  else if (!str_prefix(arg,"disease")) {
    spell = spell_cure_disease;
    sn    = ability_lookup("cure disease");
    words = "judicandus eugzagz";
    cost = 1500;
  }
  else if (!str_prefix(arg,"poison")) {
    spell = spell_cure_poison;
    sn    = ability_lookup("cure poison");
    words = "judicandus sausabru";
    cost  = 2500;
  }
  else if (!str_prefix(arg,"uncurse") || !str_prefix(arg,"curse")) {
    spell = spell_remove_curse; 
    sn    = ability_lookup("remove curse");
  ...
  if (spell == NULL) { /* restore mana trap...kinda hackish */
    ...
    if ( !str_cmp(words,"energizer" ) ) {
      //ch->mana += dice(2,8) + mob->level / 3;
      ch->mana += dice(3,8) + mob->level;
      ch->mana = UMIN(ch->mana,ch->cstat(max_mana));
      send_to_char("A warm glow passes through you.\n\r",ch);
      return;
    }
    else {
      //ch->psp += dice(2,8) + mob->level / 3;
      ch->psp += dice(3,8) + mob->level;
      ch->psp = UMIN(ch->psp,ch->cstat(max_psp));
      send_to_char("A warm glow passes through you.\n\r",ch);
      return;
    }
  }
