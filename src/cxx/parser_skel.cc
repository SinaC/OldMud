...
NonTerminal *parse_one_method( const char *s ) {
  ...
  // if we don't do this, the parser will detect a E_O_F after the method
  ...
  //initialState = 155; // we are in state 155 when we start to parse a method, grammar has been modified
  initialState = grammar_initialMethodState; // we are in state 153 when we start to parse a method
  // 148 could work too, depend if Class is abstract, doesn't really matter
